...
b){n('"'+a+'" '+b,function(a,b,c){0<b.length||0<c.length?p(c+"",b+""):p(a,null)})}for(var d={},h=Object.keys(this.options),a=0;a<h.length;a++)d[h[a].toLowerCase()]=this.options[h[a]];delete d.js;delete d.js_output_file;var k='-client -jar "'+__dirname+'/compiler/compiler.jar"';b instanceof Array||(b=[b]);for(a=0;a<b.length;a++){if("string"!=typeof b[a]||0<=b[a].indexOf('"'))throw Error("Illegal source file: "+b[a]);e=l.statSync(b[a]);if(!e.isFile())throw Error("Source file not found: "+b[a]);k+=' --js "'+
...
else throw Error("Externs file not found: "+d.a[a]);}delete d.externs;for(a=0;a<h.length;a++)k+=' --externs "'+h[a]+'"';h=Object.keys(d);for(a=0;a<h.length;a++){m=h[a];g=d[h[a]];if(!/[a-zA-Z0-9_]+/.test(m))throw Error("Illegal option: "+m);if(!0===g)k+=" --"+m;else if(!1!==g)for(g instanceof Array||(g=[g]),e=0;e<g.length;e++){if(!/[^\s]*/.test(g[e]))throw Error("Illegal value for option "+m+": "+g[e]);k+=" --"+m+" "+g[e]}}c.testJava(c.getGlobalJava(),function(a){a?q(c.getGlobalJava(),k):c.testJava(c.getBundledJava(),
